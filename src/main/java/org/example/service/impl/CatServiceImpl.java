...
public class CatServiceImpl implements CatService {
    ...
    public List<CatDto> getListByPageAndCount(int page, int count) {
        ...
        List<Cat> allObjects = catDao.getAll();
        allObjects.sort(Comparator.comparing(Cat::getId));
        ...
        List<Cat> catList = allObjects.subList(startIndex, Math.min(endIndex, allObjects.size()));
        return catList.isEmpty() ? List.of() : catList.stream().map(catConverter::convert).collect(Collectors.toList());
    }
}
